...
在 `~/.config/mote/agents/` 目录下将智能体定义为 Markdown 文件：
...
全局权限位于 `config.toml` 中，省略时默认为 `ask`。每个智能体可以在其 Markdown frontmatter 中覆盖每个工具的权限：
...
5. **智能体指令** — 来自智能体 Markdown 正文（或 `config.toml` 覆盖）
